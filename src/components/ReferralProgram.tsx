...
type RewardType = "signup" | "task_completion" | "others";
...
interface Referral {
  ...
  user_profile?: SerializedUserProfile;
  referred_name: string | null;
}

interface SerializedReferral {
  id: string;
  referrer_id: string;
  referred_id: string;
  referred_at: string;
...
interface SerializedReferralReward {
  id: string;
  reward_amount: number;
  reward_type: RewardType;
  reward_timestamp: string;
...
const RewardItem = ({
  ...
  const formatRewardType = (type: RewardType) => {
    switch (type) {
      case "signup":
  ...
  const getTierBadgeColor = (tierLevel: TierLevel) => {
    switch (tierLevel) {
      case "tier_1":
        return "bg-[#0361DA]";
      case "tier_2":
...
export const ReferralProgram: React.FC = (): JSX.Element => {
  ...
  const userProfile = useSelector((state: RootState) => {
    const profile = state.session.userProfile;
    if (!profile) return null;
    const serialized: SerializedUserProfile = {
      id: String(profile.id),
      user_name: profile.user_name || null,
      wallet_address: String(profile.wallet_address),
      referral_code: profile.referral_code ? String(profile.referral_code) : undefined,
  ...
  const referrals = useSelector((state: RootState) => {
    return state.session.referrals?.map(ref => {
      const serializedReferral: SerializedReferral = {
        id: String(ref.id),
        referrer_id: String(ref.referrer_id),
        referred_id: String(ref.referred_id),
        referred_at: String(ref.referred_at),
        tier_level: String(ref.tier_level) as TierLevel,
        user_profile: ref.user_profile ? {
          id: String(ref.user_profile.id),
          user_name: ref.user_profile.user_name || null,
          wallet_address: String(ref.user_profile.wallet_address),
          email: ref.user_profile.email || undefined
        } : undefined,
  ...
  const referralRewards = useSelector((state: RootState) => {
    return state.session.referralRewards?.map(reward => {
      const serializedReward: SerializedReferralReward = {
        id: String(reward.id),
        reward_amount: Number(reward.reward_amount),
        reward_type: String(reward.reward_type) as RewardType,
        reward_timestamp: String(reward.reward_timestamp),
        ...
        referral: reward.referral ? {
          id: String(reward.referral.id),
          referrer_id: String(reward.referral.referrer_id),
          referred_id: String(reward.referral.referred_id),
          referred_at: String(reward.referral.referred_at),
          tier_level: String(reward.referral.tier_level) as TierLevel,
          user_profile: reward.referral.user_profile ? {
            id: String(reward.referral.user_profile.id),
            user_name: reward.referral.user_profile.user_name || null,
            wallet_address: String(reward.referral.user_profile.wallet_address),
            email: reward.referral.user_profile.email || undefined
          } : undefined,
          referred_name: reward.referral.referred_name || null
        } : undefined
      };
  ...
  const tier1Referrals =
    referrals?.filter((ref) => ref.tier_level === "tier_1" as TierLevel) || [];
  const tier2Referrals =
    referrals?.filter((ref) => ref.tier_level === "tier_2" as TierLevel) || [];
  const tier3Referrals =
    referrals?.filter((ref) => ref.tier_level === "tier_3" as TierLevel) || [];
