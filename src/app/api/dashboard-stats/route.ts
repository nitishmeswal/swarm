  ...
  try {
    ...
    try {
      ...
      if (!response.ok || (data.total_users === 0 && data.total_compute_generated === 0 && data.total_tasks === 0)) {
        ...
        // Get total users count - for dashboard "Active Nodes" (showing total users)
        const { count: totalUsersCount, error: usersCountError } = await adminClient
          .from('user_profiles')
          .select('*', { count: 'exact', head: true });

        if (!usersCountError) {
          data.total_users = totalUsersCount || 0;
          console.log('Total users found:', data.total_users);
        }
        ...
        if (!globalStatsError && globalStatsData) {
          ...
          // Fallback: Calculate compute from devices uptime
          const { data: devicesData, error: devicesError } = await adminClient
            .from('devices')
            .select('uptime');

          if (!devicesError && devicesData) {
            data.total_compute_generated = devicesData.reduce((sum, device) => sum + (device.uptime || 0), 0) / 1000;
